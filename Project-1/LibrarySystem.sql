...
-- 5. List books priced above 30 LE. 
SELECT * FROM Book WHERE Price > 30;

-- 6. List members who joined before 2023. 
SELECT * FROM Members WHERE YEAR(Membership_Start_Date) < 2023;

-- 7. Display names and roles of staff working in 'Bahla Library'. 
SELECT s.Full_Name, s.Position
FROM Staff s
JOIN Librarys l ON s.L_ID = l.L_ID
WHERE l.L_Name = 'Bahla Library';

-- 8. Display branch name managed by staff ID = 2. 
SELECT l.L_Name
FROM Librarys l
JOIN Staff s ON s.L_ID = l.L_ID
WHERE s.S_ID = 2;
...
INSERT INTO Members (M_ID, Full_Name, Email, Phone, Membership_Start_Date)
VALUES (405, 'Your Name', 'your@email.com', '12345678', GETDATE());
SET IDENTITY_INSERT Members OFF;

-- Now borrow book ID = 1011
INSERT INTO Loan_Link (B_ID, M_ID, Loan_Date, Due_Date, Return_Date, L_Status)
VALUES (1011, 405, GETDATE(), DATEADD(DAY, 14, GETDATE()), NULL, 'Issued');


-- 11. Insert a member with NULL email and phone.
INSERT INTO Members (Full_Name, Email, Phone, Membership_Start_Date)
VALUES ('Member With Nulls', NULL, NULL, GETDATE());
